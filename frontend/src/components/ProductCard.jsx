...
import EcoDots from "./ui/EcoDots.jsx";
...
export default function ProductCard({
  ...
  const titleCls = darkSection
    ? "font-display text-[1.31rem] font-bold text-white leading-[1.19] tracking-[0.231px] line-clamp-2"
    : "font-display text-[1.31rem] font-bold text-ink-950 leading-[1.19] tracking-[0.231px] line-clamp-2";
  const subtitleCls = darkSection
    ? "text-caption text-white/75 mt-1 line-clamp-1 tracking-[-0.224px]"
    : "text-caption text-apple-textSecondary mt-1 line-clamp-1 tracking-[-0.224px]";
  const descCls = darkSection
    ? "text-caption text-white/65 line-clamp-2 mt-2 leading-[1.29] tracking-[-0.224px]"
    : "text-caption text-apple-textSecondary line-clamp-2 mt-2 leading-[1.29] tracking-[-0.224px]";
  const priceCls = darkSection
    ? "text-[1.31rem] font-display font-semibold text-white tabular-nums shrink-0"
    : "text-[1.31rem] font-display font-semibold text-ink-950 tabular-nums shrink-0";
  const ecoLabelCls = darkSection
    ? "text-micro text-white/70 mt-1"
    : "text-micro text-apple-textSecondary mt-1";
  ...
  return (
    ...
    >
      <div className={`relative ${imageAspectClass} bg-apple-gray overflow-hidden shrink-0`}>
        ...
        {stock.tone === "out" && (
          <div
            className={`absolute inset-0 pointer-events-none ${darkSection ? "bg-black/35" : "bg-black/20"}`}
            aria-hidden
          />
        )}
      </div>
      ...
      <div
        className={`p-5 flex flex-col flex-1 min-h-0 ${darkSection ? "bg-apple-surface1" : "bg-white"}`}
      >
        ...
        <div className="mt-auto pt-4 flex items-end justify-between gap-3">
          <div data-testid={`product-card-eco-${p.id}`}>
            ...
            <p className={ecoLabelCls}>Green index: {p.ecoScore ?? 0}/5</p>
          </div>
        ...
        {!featured && (
          <p
            className={`text-micro mt-3 ${darkSection ? "text-apple-linkDark group-hover:underline" : "text-apple-link group-hover:underline"}`}
          >
            View details →
          </p>
        )}
